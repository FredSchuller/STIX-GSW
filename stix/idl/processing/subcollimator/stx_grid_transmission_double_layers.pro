...
; NAME:
;
;   stx_grid_transmission_double_layers
;
; PURPOSE:
;
...
; CALLING SEQUENCE:
;
;   int_shadow = stx_grid_transmission_double_layers(x_flare, y_flare, grid_orient, grid_pitch, grid_slit, grid_thick)
;
; INPUTS:
; 
...
function stx_grid_transmission_double_layers, x_flare, y_flare, grid_orient, grid_pitch, grid_slit, mean_thick
  ...
  prova = average_slit - average_pitch/2.
  
  this_pitch = this_slat + this_slit
  ...
  ;; Internal shadowing
  shadow_width = (mean_thick/2.)  * tan(flare_dist / 3600. * !dtor)
